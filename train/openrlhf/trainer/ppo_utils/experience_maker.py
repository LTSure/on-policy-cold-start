...
# [lhy add]
class NaiveExperienceMakerSFT_MT(ABC):
    """
    Naive experience maker.
    """
    ...
    def generate_samples(self, all_prompts: List[str], all_answers: List[str], all_responses: List[str], **generate_kwargs) -> List[Samples]:
        ...
        for i in range(0, len(all_prompts), args.micro_rollout_batch_size):
            ...
            def process_multi_turn_sequences(prompts):
                sequences = self.tokenize_fn(prompts, generate_kwargs.get("max_new_tokens"), device="cuda", padding_side="right")['input_ids']
...
class NaiveExperienceMakerPRM800K(ABC):
    ...
    @torch.no_grad()
    def make_experience_MT(self, envs, **generate_kwargs) -> Experience:
        self.actor.eval()
        self.initial_model.eval()
        ...
        kls = []
        total_scores = []
        sequences, attention_masks, action_masks, action_log_probs, base_action_log_probs, advantages = [], [], [], [], [], []
        for env in envs:
            # print("collect env")
            last_act = ""
            action_idxs, state_idxs = [], []
            conversations = []
            conversations += prompt
            
            obs, info = env.reset()
            obs_content = {"role": "user", "content": obs[0][len("-= Welcome to TextWorld, ALFRED! =-\n\n") :]}
            # conversations.append(obs_content)
            while True:
                conversations.append(obs_content)
                prompts = self.apply_chat_template(conversations)
                inputs = self.tokenize_fn(prompts, self.prompt_max_len, device="cuda")
                input_length = inputs["input_ids"].shape[-1]
                state_idxs.append(input_length - 1)
                if input_length >= self.prompt_max_len:
                    score = 0
                    break
                ...
                generate_args = {
                    ...
                    "early_stopping": True,
                    "temperature": generate_kwargs.get("temperature", 1),
                    "use_cache": True,
                    "num_beams": generate_kwargs.get("num_beams", 1),
                    "attention_mask": inputs["attention_mask"], #generate_kwargs.get("attention_mask"),
                    "eos_token_id": generate_kwargs.get("eos_token_id"),
                    "pad_token_id": generate_kwargs.get("pad_token_id"),
                    "min_new_tokens": generate_kwargs.get("min_new_tokens", 1),
                }

                if generate_kwargs.get("max_new_tokens", None):
                    generate_args["max_new_tokens"] = generate_kwargs.get("max_new_tokens")
                if generate_kwargs.get("max_length", None):
                    generate_args["max_length"] = generate_kwargs.get("max_length")
                    
                sequence = self.actor.model.generate(**generate_args)
                output_length = sequence.shape[-1]
                action_idxs.append(output_length - 1)
                
                # print(input_length, output_length)
                
                outputs = sequence[:, input_length:]
                text = self.tokenizer.batch_decode(outputs, skip_special_tokens=True)[0]
                
                action_content = {"role": "assistant", "content": text}
                conversations.append(action_content)
                
                acts = [a for a in text.split("Action: ")]
                if len(acts) <= 1:
                    print("warning action", text, len(prompts), len(text))
                    score = -0.0
                    break
                act = acts[1]
                ...
                if ob.startswith("You arrive at loc "):
                    ob = ob[ob.find(". ") + 2 :]
                obs_content = {"role": "user", "content": "Observation: " + ob}
            ...
            # print(sequence.shape, "before")
            sequence = sequence[:, :self.prompt_max_len]
            # print(sequence.shape, "after")
            attention_mask = sequence.ne(pad_token_id).to(dtype=torch.long)
            action_mask = torch.zeros_like(attention_mask)
            for num in range(len(action_idxs)):
                action_mask[:, state_idxs[num]:action_idxs[num]] = 1
            ...
            # log probs
            outputs = self.actor.model(sequence, attention_mask=attention_mask)
            base_outputs = self.initial_model.model(sequence, attention_mask=attention_mask)
            sequence_next = torch.cat([sequence[:, 1:], sequence[:, -1:]], dim=-1)
            
            action_log_prob = log_probs_from_logits(outputs["logits"], sequence_next)
            base_action_log_prob = log_probs_from_logits(base_outputs["logits"], sequence_next)
            
            # rewards
            reward = torch.tensor([float(score) for _ in range(sequence.shape[-1])]).to(sequence.device).unsqueeze(0)
            log_ratio = action_log_prob - base_action_log_prob
            use_k3 = True
            if not use_k3:
                kl = -self.kl_ctl.value * log_ratio
            else:
                log_ratio = -log_ratio
                kl = -self.kl_ctl.value * (log_ratio.exp() - 1 - log_ratio)
            advantage = reward + kl
            ...
            base_action_log_probs.append(base_action_log_prob)
        ...
        def zero_pad_sequences(sequences, side: str = "left", value=0):
            ...
            for seq in sequences:
                ...
                padding = (pad_len, 0) if side == "left" else (0, pad_len)
                padded_sequences.append(F.pad(seq, padding, value=value))
            return torch.stack(padded_sequences, dim=0).squeeze(1)
        
        sequences = zero_pad_sequences(sequences, "left", value=self.tokenizer.pad_token_id)
        attention_masks = zero_pad_sequences(attention_masks, "left", value=0)
        action_masks = zero_pad_sequences(action_masks, "left", 0).bool()
        action_log_probs = zero_pad_sequences(action_log_probs, "left", 0.).type(action_log_probs[0].dtype)
        base_action_log_probs = zero_pad_sequences(base_action_log_probs, "left", 0.).type(base_action_log_probs[0].dtype)
        advantages = zero_pad_sequences(advantages, "left", 0.).type(advantages[0].dtype)
        
        # sequences = torch.cat(sequences, dim=0)
        # attention_masks = torch.cat(attention_masks, dim=0)
        # action_masks = torch.cat(action_masks, dim=0)
        # action_log_probs = torch.cat(action_log_probs, dim=0)
        # base_action_log_probs = torch.cat(base_action_log_probs, dim=0)
        # advantages = torch.cat(advantages, dim=0)
        
        kls = zero_pad_sequences(kls, "left", 0.).type(kls[0].dtype)
        r = torch.tensor([float(score) for _ in total_scores]).to(sequence.device)

        info = {
            "kl": masked_mean(kls, action_masks, dim=-1),
            "reward": r,
            "return": masked_mean(advantages, action_masks, dim=-1),
            "response_length": action_masks.float().sum(dim=-1),
            "total_length": attention_masks.float().sum(dim=-1),
        }
        # reset model state
        value, returns = advantages.clone(), advantages.clone()
        # print(sequences.shape, advantages.shape, attention_masks.shape)
        self.actor.train()
