...
def preprocess_data_box(data, input_template=None, input_key="input", apply_chat_template=None, use_muti_turn=False, tokenizer=None, prompt_data="") -> str:
    if apply_chat_template:
        if not use_muti_turn:
            ...
            truth_answer = data.get("ground_truth_answer", "")
            target = data.get("target", "")
            
            if input_template:
                prompt = input_template.format(prompt)

            return {"input": prompt, "target": target, "answer": truth_answer, "response": response}
...
def train(args):
    ...
    actor = Actor(
        ...
        use_muti_turn=args.use_muti_turn,
    ...
    trainer = SFTPPOTrainer(
        ...
        without_ppo=args.no_ppo,
        use_decay=args.use_decay,
        loss_mode=args.loss_mode,
        use_muti_turn=args.use_muti_turn,
        dft_baseline=args.dft_baseline,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no_ppo", action="store_true", default=False)
    parser.add_argument("--dft_baseline", action="store_true", default=False, help="Enable DFT baseline mode for confidence-based loss reweighting")
    parser.add_argument("--neftune_alpha", type=float, default=None, help="NEFT alpha parameter for embedding noise injection (prevents overfitting)")
    ...
    parser.add_argument("--buffer_limit", type=int, default=0, help="buffer_limit")
    ...
    parser.add_argument("--init_kl_coef", type=float, default=0.00, help="KL penalty in PPO")
    parser.add_argument("--entropy_coef", type=float, default=0.00, help="KL penalty in PPO")
    parser.add_argument(
        "--use_kl_estimator_k3",
        action="store_true",
        default=False,
    ...
    parser.add_argument("--use_decay",action="store_true",help="use clip_eps decay")
    parser.add_argument("--loss_mode",type=str, default="clip",help="loss mode")
    parser.add_argument("--use_muti_turn",action="store_true")
    parser.add_argument("--save_gradient_norms", action="store_true", default=False, help="Save individual gradient norm values for each step to text file")
    parser.add_argument("--save_step_probs", action="store_true", default=False, help="Save step probabilities for each batch to text file")
    parser.add_argument("--save_prob_gradient_pairs", action="store_true", default=False, help="Save token probability-gradient pairs for analysis")
